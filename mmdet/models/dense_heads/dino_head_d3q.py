...
class DINOHead_D3Q(DeformableDETRHead):
    ...
    def __init__(self,
        ...
        self.d_bg_cls_weight=0
        class_weight = loss_d_cls.get('class_weight', None)
        self.loss_d_activated = loss_d_cls.get('activated', False)
        loss_d_cls.pop('activated')
    ...
    def loss_densitymap_head(self, densitymap_pred: Tensor, batch_gt_instances: InstanceList,
        ...
        densitymap_target, avg_factor = self.get_targets_densitymap_head(gt_bboxes, gt_labels,
                                                     ...
                                                     img_shape)
        # a = torch.sum(densitymap_target,dim=(1, 2, 3))
    ...
    def loss(self, hidden_states: Tensor, references: List[Tensor],
             enc_outputs_class: Tensor, enc_outputs_coord: Tensor, densitymap_pred: Tensor, 
        ...
        losses = self.loss_by_feat(*loss_inputs, densitymap_pred=densitymap_pred)
        return losses
    ...
    def _loss_dn_single(self, dn_cls_scores: Tensor, dn_bbox_preds: Tensor,
        ...
        # classification loss
        if self.loss_d_activated:
            cls_scores = dn_cls_scores.reshape(-1, self.d_cls_out_channels)
            cls_avg_factor = \
            num_total_pos * 1.0 + num_total_neg * self.d_bg_cls_weight
        else:
    ...
    def _predict_by_feat_single(self,
        ...
        # exclude background
        if self.loss_d_cls.use_sigmoid:
