...
@weighted_loss
def gaussian_focal_loss_t(pred: Tensor,
    ...
    neg_weights = gaussian_target.eq(0)
    pos_weights = ~neg_weights
    pos_loss = -(gaussian_target*(pred + eps).log() + (1 - gaussian_target)*(1 - pred + eps).log()) * (gaussian_target - pred).abs().pow(alpha) * pos_weights
    neg_loss = -(1 - pred + eps).log() * pred.pow(alpha) * neg_weights

    return pos_weight * pos_loss + neg_weight * neg_loss


def gaussian_focal_loss_t_with_pos_inds(
        pred: Tensor,
        gaussian_target: Tensor,
        pos_inds: Tensor,
        pos_labels: Tensor,
...
class GaussianFocalLoss_T(nn.Module):
    ...
    def forward(self,
        ...
        if pos_inds is not None:
            ...
            loss_reg = self.loss_weight * gaussian_focal_loss_t_with_pos_inds(
                pred,
                target,
                pos_inds,
                pos_labels,
            ...
            loss_reg = self.loss_weight * gaussian_focal_loss_t(
                pred,
                target,
                weight,
                alpha=self.alpha,
