...
from ..layers import (DQDeformableDetrTransformerDecoder2,
                      DeformableDetrTransformerEncoder, SinePositionalEncoding)
from .base_detr import DetectionTransformer
from ..utils import multi_apply

@MODELS.register_module()
class DeformableDETR_D3Q(DetectionTransformer):
    ...
    def __init__(self,
                ...
                #  feature_size: int = 100^2+50^2+25^2+13^2,
        ...
        self.current_query_num = 0
        self.result_file = 'gt_query_num_r.npy'
    ...
    def _init_layers(self) -> None:
        ...
        self.decoder = DQDeformableDetrTransformerDecoder2(**self.decoder)
        ...
        if self.as_two_stage:
            ...
            # self.query_num1 = nn.Linear(self.embed_dims, 1)
            # self.query_num2 = nn.Linear(self.feature_size, 1)
            # self.query_num3 = nn.ReLU()
    ...
    def init_weights(self) -> None:
        ...
        if self.as_two_stage:
            ...
            # nn.init.xavier_uniform_(self.query_num1.weight)
            # nn.init.xavier_uniform_(self.query_num2.weight)
        else:
            xavier_init(
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        head_inputs_dict, densitymap_pred, query_num = self.forward_transformer(img_feats,
                                                    batch_data_samples)
        num_loss_input=dict(
            densitymap_pred=densitymap_pred,
            query_num=query_num)
        head_inputs_dict.update(num_loss_input)
    ...
    def predict(self,
        ...
        """
        img_feats = self.extract_feat(batch_inputs)
        head_inputs_dict, _, query_num = self.forward_transformer(img_feats,
                                                    batch_data_samples)
        num_loss_input=dict(
            query_num=query_num)
        head_inputs_dict.update(num_loss_input)
        ...
        # import numpy as np
        # load_data = list(np.load(self.result_file))
        # save_data = [int(batch_data_samples[0].gt_instances.labels.shape[0]), int(self.current_query_num.cpu())]
        # load_data.append(save_data)
        # np.save(self.result_file, np.array(load_data))
    ...
    def forward_transformer(self,
        ...
        tmp_dec_in, head_inputs_dict, densitymap_pred, query_num = self.pre_decoder(**encoder_outputs_dict)
        decoder_inputs_dict.update(tmp_dec_in)

        decoder_outputs_dict = self.forward_decoder(**decoder_inputs_dict)
        head_inputs_dict.update(decoder_outputs_dict)
        return head_inputs_dict, densitymap_pred, query_num
    ...
    def pre_decoder(self, memory: Tensor, memory_mask: Tensor,
        ...
        if self.as_two_stage:
            ...
            # enc_outputs_class_sort = enc_outputs_class[..., 0].sigmoid()
            # b = F.softmax(enc_outputs_class[..., 0], 1)
            # b_mean = torch.mean(b, -1)
            # b_std = torch.std(b, -1)
            # b_t = b_mean + b_std
            # b_k = (b > b_t.unsqueeze(-1)) *2
            # b_k = torch.sum(b_k, 1)

            # enc_outputs_class_mean = torch.mean(enc_outputs_class_sort, -1)
            # enc_outputs_class_std = torch.std(enc_outputs_class_sort, -1)
            # enc_outputs_k_t = enc_outputs_class_mean + enc_outputs_class_std
            # mask_k = (enc_outputs_class_sort > enc_outputs_k_t.unsqueeze(-1)) *2
            # mask_k = torch.sum(mask_k, 1)
            # mask_k = torch.clamp(mask_k, 100, self.num_queries)
            
            densitymap_pred = self.bbox_head.density_map_forward(output_memory, spatial_shapes)
            densitymap_pred_num = densitymap_pred.detach()
            query_num = torch.sum(densitymap_pred_num,dim=(1, 2, 3))
            query_num = torch.round(query_num)

            # self.current_query_num = query_num #for debug and save result

            # tmp_query_num = self.query_num1(output_memory) #（bs，num_feature, 1）
            # tmp_query_num1 = self.query_num2(tmp_query_num.squeeze(2))  #（bs， 1）
            # query_num = torch.round(tmp_query_num1.squeeze(1))   #（bs）

            query_num = query_num*self.t
            if self.min_queries is not None:
                query_num = torch.clamp(query_num, self.min_queries, self.max_queries)
            else:
                query_num = torch.clamp(query_num, 1, self.max_queries)
            ...
            for i in range(batch_size):
                topk_proposals = torch.topk(
                    enc_outputs_class[i,..., 0], int(query_num[i].item()), dim=0)[1]
                topk_coords_unact = torch.gather(
                    enc_outputs_coord_unact[i,...], 0,
                    topk_proposals.unsqueeze(-1).repeat(1,  4))
                topk_coords_unact = topk_coords_unact.detach()
                d_reference_points = topk_coords_unact.sigmoid()
                # pad_reference_points = d_reference_points.new_zeros(self.num_queries, d_reference_points.shape[1])
                # pad_reference_points[:query_num[i],...] = d_reference_points
                pos_trans_out = self.pos_trans_fc(
                    self.get_proposal_pos_embed(topk_coords_unact.unsqueeze(0),num_pos_feats=64))
                d_query_pos = self.pos_trans_norm(pos_trans_out)
                # d_query_pos, d_query = torch.split(pos_trans_out, c, dim=2)
                d_query = self.query_embedding.weight[None, :int(query_num[i].item()), :]

                # pad_pos_trans_out = pos_trans_out.new_zeros(self.num_queries, pos_trans_out.shape[1])
                # pad_pos_trans_out[:query_num[i],...] = pos_trans_out
                query.append(d_query)
                query_pos.append(d_query_pos)
                reference_points.append(d_reference_points.unsqueeze(0))
                # memory_list.append(memory[i].unsqueeze(0))
            # pad_pos_trans_outs= torch.stack(pad_pos_trans_outs, 0)
            # reference_points= torch.stack(reference_points, 0)
            # query_pos, query = torch.split(pad_pos_trans_outs, c, dim=2)
        ...
        return decoder_inputs_dict, head_inputs_dict, densitymap_pred, query_num

    def forward_decoder(self, query: List[Tensor], query_pos: List[Tensor], memory: Tensor,
                        memory_mask: Tensor, reference_points: List[Tensor],
        ...
        #  (bs, num_decoder_layers, 1, num_queries, 4)
        inter_states, inter_references = multi_apply(
            self.decoder,
            query,
            query_pos,
            reference_points,
        ...
        references = []
        for i in range(batch_size):
            references_i = [reference_points[i], *(inter_references[i])]
            references.append(references_i)
